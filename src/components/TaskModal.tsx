...
import { X, Calendar, User, Tag, AlertCircle, Flag, Loader2, MessageSquare, FileEdit, Paperclip } from 'lucide-react';
import type { Task, TaskInput, TaskCategory, TaskStatus, TaskPriority } from '../hooks/useTasks';
import { supabase } from '../lib/supabase';
import { useComments, useAttachments } from '../hooks';
...
export function TaskModal({ isOpen, onClose, onSubmit, task, mode }: TaskModalProps) {
    ...
    const taskIdForHooks = mode === 'edit' ? task?.id || null : null;
    const { comments, loading: commentsLoading, createComment, updateComment, deleteComment, currentUserId } = useComments(taskIdForHooks);
    const { attachments, loading: attachmentsLoading, uploading, uploadFile, deleteAttachment } = useAttachments(taskIdForHooks);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const taskData: TaskInput = {
            ...
            assignee_ids: selectedAssignees.map((a: any) => a.id),
            assignee_names: selectedAssignees.map((a: any) => a.full_name || a.email),
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-lg max-h-[85vh] bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden animate-slide-up flex flex-col">
                ...
                {mode === 'edit' && (
                    <div className="flex-shrink-0 flex border-b border-slate-200 dark:border-slate-700">
                        <button
                            ...
                            className={`flex items-center gap-2 px-6 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === 'details'
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-6 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === 'comments'
                                ? 'border-primary-500 text-primary-500'
                                : 'border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'
                                }`}
                        >
                            ...
                            Comments
                            {comments.length > 0 && (
                                <span className="ml-1 px-1.5 py-0.5 bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-400 text-xs rounded-full">
                                    {comments.length}
                                </span>
                            )}
                        </button>
                        <button
                            ...
                            className={`flex items-center gap-2 px-6 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === 'attachments'
                                ? 'border-primary-500 text-primary-500'
                                : 'border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'
                                }`}
                        >
                ...
                {mode === 'edit' && activeTab === 'comments' && (
                    <div className="flex-1 p-6 overflow-hidden">
                        <CommentThread
                            ...
                            onAddComment={async (content, parentId) => createComment({ content, parent_id: parentId })}
                ...
                {mode === 'edit' && activeTab === 'attachments' && (
                        ...
                        <div className="relative">
                                ...
                                <div className="flex flex-wrap gap-2 mb-2">
                                    {selectedAssignees.map((user) => (
                                        ...
                                        >
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => setSelectedAssignees((prev: any) => prev.filter((a: any) => a.id !== user.id))}
                            ...
                            {showMentions && (
                                ...
                                >
                                    ...
                                    ) : filteredUsers.filter((u: any) => !selectedAssignees.find((s: any) => s.id === u.id)).length === 0 ? (
                                        <div className="py-3 px-4 text-sm text-slate-500 dark:text-slate-400 text-center">
                                            No users found
                                        </div>
                                    ) : (
                                        filteredUsers
                                            .filter((u: any) => !selectedAssignees.find((s: any) => s.id === u.id))
                                            .map((user: any) => (
                                                <button
                                                    key={user.id}
                                                    type="button"
                                                    onClick={() => {
                                                        setSelectedAssignees((prev: any) => [...prev, user]);
                                                        setAssigneeName('');
                                                        setShowMentions(false);
                                                    }}
